...
public class MyGraph implements Graph
{
    ...
    protected int addThePath(Path path)
    {
        pmap.put(path,++count);
        imap.put(count,path);
        return count;
    ...
    protected int remPath(Path path)
    {
    ...
    protected Path remId(int id)
    {
        Path path = imap.remove(id);
        pmap.remove(path);
        return path;
    }
    
    public HashSet<Integer> getNodemap()
    {
        return new HashSet<>(nodemap.keySet());
    }
    
    public HashSet<Path> getPmap()
    {
        return new HashSet<>(pmap.keySet());
    }
    ...
    protected void addNode(Path path)
    {
        int i;
        int node;
        int times;
    ...
    protected void removeNode(Path path)
    {
        for (int i = 0;i < path.size();i++)
        {
            int node = path.getNode(i);
            int times = nodemap.get(node) - 1;
            nodemap.remove(node);
            if (times >= 1)
            {
                nodemap.put(node,times);
